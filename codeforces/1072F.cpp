...
bool dp[200005][3];
void dfs(ll node,ll par,vector<vll>&adj) {
    bool isL=true;
    bool cr[3]={true,false,false};
    for (auto ch: adj[node]) {
        if (ch == par) continue;
        isL=false;
        dfs(ch,node,adj);
        bool ns[3]={false, false, false};
        for (int i=0; i<3;i++) {
            if (cr[i]) {
                for (int j=0; j<3;j++) {
                    if (dp[ch][j]) {
        ...
        for(int i=0;i<3;i++) cr[i]=ns[i];
    }

    dp[node][0]=dp[node][1]=dp[node][2]=false;

    if(isL) {
        dp[node][1] = true;
    } else {
        for (int i=0;i<3;i++) dp[node][i] = cr[i];
        dp[node][1] = true;
    }
}
...
void solve() {
    ...
    for(int i=0; i<=n; i++) dp[i][0] = dp[i][1] = dp[i][2] = false;
    dfs(1,-1,adj);
