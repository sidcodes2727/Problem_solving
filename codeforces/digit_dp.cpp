...
// count numbers less than n that do not contain the digit 4

int dp[20][2][2];

int f(string s, int i , bool tight, bool found4){
    if(i==s.size()) return found4 == 0;
    if(dp[i][tight][found4]!=-1)    return dp[i][tight][found4];
    int limit= tight ? s[i] - '0' : 9;
    int ans=0;
    for(int d=0;d<=limit;d++){
        int newFound4= found4 || (d==4);
        int newTight= tight && (d==(s[i]-'0'));
        ans+= f(s, i+1 , newTight, newFound4);
    }
    return dp[i][tight][found4]=ans;
}
void sid() {
    int l,r;
    cin>>l>>r;
    ...
    cout<< f(s1,0,1,0) + f(s2,0,1,0) << endl;
}
