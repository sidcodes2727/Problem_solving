...
void solve() {
    ...
    vector<ll> res;
    vector<ll> temp;
    for(auto i:s)   temp.pb(i);
    vector<bool> cov(temp.size(),0);
    for(int j=0;j<temp.size();j++){
        ...
        res.pb(i);
        for(ll mul=i;mul<=k;mul+=i){
            auto it=lower_bound(all(temp),mul);
            if(it!=temp.end() && *it==mul){
