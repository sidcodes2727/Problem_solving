...
void solve() {
    ...
    map<int,int> conclude;

    for(auto i:mp){
        int siz=i.second.size();
        vector<int> temp=i.second;
        int mi = max(temp[0] , n - temp[siz-1]-1);
        for(int idx=1;idx<siz;idx++){
            mi=max(mi, temp[idx]-temp[idx-1]-1);
        }
        conclude[i.first]=mi;
    }
    debug(mp);
    ...
    vector<int> ans(n,-1);
    int prev = n+1;
    for(auto it=conclude.begin();it!=conclude.end();it++){
        int val=it->first;
        int gap=it->second;
        int mx=gap+1;
        for(int k=mx; k<prev;k++){
            if(ans[k-1]==-1)    ans[k-1]=val;
        }
        prev=mx;
    }
