...
string nthPermutation(string s, ll n) {
    sort(s.begin(), s.end());  
    int len = s.size();
    vector<ll> fact(len, 1);
...
void solve() {
    ...
    string n1=nthPermutation(to_string(n),j);
    string n2=nthPermutation(to_string(n),k);
    int x=0;
    int siz=n1.size();
    for(int i=0;i<siz;i++)  x+=(n1[i]==n2[i]);
    set<char> inter(n1.begin(),n1.end());
    int y=0;
    for(auto i:n2){
        if(inter.find(i)!=inter.end())  y++;
    }
    y-=x;
    cout<<x<<"A"<<y<<"B"<<endl;
